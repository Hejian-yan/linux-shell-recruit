...
`$0`脚本本身名字，`$2` = 第 2 个参数，以此类推，``$@`把所有`$1 $2 $3 ...`` 拿出来
...
| 写法   | 行为                                                         |
| ------ | ------------------------------------------------------------ |
| `$@`   | 不带引号：**会分词、通配符展开**。参数内部空格会被切碎，一个参数拆成多个。 |
| `"$@"` | 带双引号：**原样保留每一个参数**，每个参数当作独立整体。内部带空格的参数不会被拆分。 |

位置参数：执行脚本时，命令行传给脚本的参数，按 1、2、3 顺序编号

shift :丢掉第一个位置参数 $1，剩下参数整体往前挪一位
...
```
for item in apple banana "hello world"; do   
  echo "$item"
done
```
...
![typora_image](typora_image.png)

## 心得体会

面对陌生的 Linux 环境，不应该急于直接写完整命令或脚本。先观察环境用ls，pwd定位，再选用各司其职的小工具，依靠管道与重定向（>,>>)串联数据流，把重复工作封装成脚本，最后做好测试与边界处理。shell是一个调度这，把用户的指令转换成一个个实用的小工具来解决问题
